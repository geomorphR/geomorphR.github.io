...
.pull-left[
+ **Note: concepts of positive and negative allometry are only applicable to univariate traits relative to another trait!** 
  + For multivariate traits, allometry is a vector in multi-dimensional space where 'positive' and 'negative' as directions are not defined and have no meaning.
  + It is mathematically impossible for all body parts to grow simultaneously faster (positive) or slower (negative) than total body size.
]
...
where $\small\mu_{\log\left(F\right)}$ and $\sigma_{\log\left(F\right)}$ are the expected value and standard deviation from the normalized sampling distribution, respectively.
]
...
+ Many biological hypotheses are related to the comparison of allometries
...
+ Do allometries have similar starting points or ending points?

+ Are allometric trajectories similar? (parallelism)

+ Do allometric trajectories converge or diverge?

+ Does the slope of the trajectory correspond with some *a priori* value?

--

+ Hypothesis-specific quantitative summary measures plus RRPP can be used to address these hypotheses
...
### Comparing Allometric Trajectories to *a priori* Pattern 

+ Sometimes we wish to know whether our trajectory follows a hypothesized pattern
  + Does my trajectory follow an isometric pattern? 
  + Does my trajectory follow a $\frac{3}{4}$ power law?
  
+ Such patterns may be evaluated using the coefficient vector, $\beta$ 
  + Compare $\beta$ to $\beta_{exp}$ using RRPP 

---

### Comparing Allometric Trajectories to *a priori* Pattern $^1$

+ RRPP procedure 
  + Fit data to $H_A$; obtain $\beta_{obs}$ and test statistic $d = |\beta_{obs} - \beta_{exp}|$
  + Fit data to $H_{0_{exp}}$, containing $\beta_{exp}$; obtain $\hat{\textbf{y}}_{exp}$ and $\textbf{e}_{exp}$
  + Permute $\textbf{e}_{exp}$, obtain pseudovalues: $\mathcal{Y} = \hat{\textbf{y}}_{exp} + \textbf{e}_{exp}^*$
  + Evaluate $H_A$ for $\mathcal{Y}$, obtain $\beta_{rand}$
  + Obtain test statistic: $d = |\beta_{rand} - \beta_{exp}|$
  + Repeat to obtain empirical sampling distribution for $d$

+ Method is made more general by standardizing as: $d_M= \frac{\beta_{obs}-\beta_{exp}}{MSE}$
  + where MSE is from $H_A$
+ Procedure is implemented in `betaTest` in `RRPP`

.footnote[
1: Tejero-Cicuéndez et al. (2023)
]

---

### Comparing Allometric Trajectories to *a priori* Pattern: Example $^1$

+ Does intraspecific allometry correspond to evolutionary 'lines of least resistance'?
  + Analysis of allometric trajectories relative to isometry (here vector of 1s)
+ Allometry differed from isometry, and was distinct across habitat types

.pull-left[

```{r, echo = FALSE, fig.align = "center",out.width="75%"}
include_graphics("LectureData/07.allometry/Tejero2023-Data.png")  
```
...
.pull-right[
```{r, echo = FALSE, fig.align = "center",out.width="75%"}
include_graphics("LectureData/07.allometry/Tejero2023-Res.png")  
```

]

.footnote[1: Tejero-Cicuéndez et al. (2023)]

---

### Comparing Allometric Trajectories to *a priori* Pattern: betaTest

+ Some GM data vs. isometry (not overly informative as the linear model tests the same thing, but serves to show `betaTest`)
...
summary(bt) #differs from isometry
```

---

### Comparing Allometric Trajectories Across Groups: Pupfish Example

+ With multiple groups, numerous questions arise: 
