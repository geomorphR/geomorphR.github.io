...
.pull-left[
```{r, echo = FALSE}
include_graphics("LectureData/06.shapestatistics1/plethExampleGPA.png")
```
]
.pull-right[
```{r, echo = FALSE}
include_graphics("LectureData/06.shapestatistics1/plethExampleFood.png")
```
]
---
...
Let $\mathbf{\tilde{Z}} = \mathbf{\Omega}^{-1/2}\mathbf{Z}$. $^2$

Then,

$$\hat{\mathbf{\beta }}=\left ( \tilde{\mathbf{X}}^{T} \tilde{\mathbf{X}}\right )^{-1}\left ( \tilde{\mathbf{X}}^{T} \mathbf{\tilde{Z}} \right)$$

.footnote[
1. Different ways of obtaining a matrix square-root are presented in the Appendix.
2. If $\mathbf{\Omega} = \mathbf{I}$, then $\mathbf{\tilde{X}} = \mathbf{X}$ and $\mathbf{\tilde{Z}} = \mathbf{Z}$, meaning this approach finds either the GLS or OLS solution to estimation.
]
...
This is the basis for all model comparisons.  This is a model that suggests there are no explanatory parameters, and that estimating the mean is sufficient for predicting what a vector of values $(\mathbf{z}^T)$ should be for an entire population of data.  
...
$$\hat{\boldsymbol{\Sigma}}_{effect} = (k-1)^{-1}(\Delta\mathbf{\tilde{H}\tilde{Z}})^T\Delta\mathbf{\tilde{H}\tilde{Z}},$$
where $\Delta\tilde{\mathbf{H}} = \tilde{\mathbf{H}}_{alt} - \tilde{\mathbf{H}}_{null}$.  This looks like a comparison of fitted values, but is, in fact, a comparison of transformed fitted values, since we use $\Delta\mathbf{\tilde{H}}$ rather than $\Delta\mathbf{H}$ and $\mathbf{\tilde{Z}}$ instead of $\mathbf{Z}$.  
...
.remark-code[.red[
If the preceding algebraic demonstration is daunting, please realize that $\Delta\mathbf{\tilde{H}\tilde{Z}}$ can be loosely translated as an effect due to differences by model predictions.  In this way, when we talk about more complex models, $\Delta\mathbf{\tilde{H}\tilde{Z}}$ is a prevailing concept.
]]

** $\Delta\mathbf{HZ}$ .red[is the difference in model predictions and] $\hat{\boldsymbol{\Sigma}}_{effect} = (k-1)^{-1}(\Delta\mathbf{\tilde{H}\tilde{Z}})^T\Delta\mathbf{\tilde{H}\tilde{Z}}$ .red[is the covariance matrix for the different predictions, the effect of adding parameters to the null model design.]**
