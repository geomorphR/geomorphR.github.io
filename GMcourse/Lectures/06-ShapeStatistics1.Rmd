...
1. The coefficients, $\hat{\boldsymbol{\beta}}$, explain the expected change in transformed data, $\mathbf{\tilde{Z}}$, associated with the *standardized matrix of parameters*.  (This is different than standardizing each of the parameters, which could also be done.)
2. Note that the linear model design has $k$ parameters (columns).  $\tilde{\mathbf{X}}^T\tilde{\mathbf{X}}$ is a $k \times k$ matrix.  $\mathbf{A} = \tilde{\mathbf{X}}(\tilde{\mathbf{X}}^T\tilde{\mathbf{X}})^{-1}$ is an $n \times k$ matrix, just like $\tilde{\mathbf{X}}$  Thus, $\hat{\boldsymbol{\beta}}$ is a $k \times p$ matrix, expressing the change in each of $p$ variables in $\mathbf{\tilde{Z}}$ associated with each of $k$ parameters in $\tilde{\mathbf{X}}$.
...
** $\Delta\mathbf{\tilde{H}\tilde{Z}}$ .red[is the difference in model predictions and] $\hat{\boldsymbol{\Sigma}}_{effect} = (k-1)^{-1}(\Delta\mathbf{\tilde{H}\tilde{Z}})^T\Delta\mathbf{\tilde{H}\tilde{Z}}$ .red[is the covariance matrix for the different predictions, the effect of adding parameters to the null model design.]**
