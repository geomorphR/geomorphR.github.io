...
### Scale and Size
.pull-left[
+ Position and orientation are due to digitizing procedures, and are (typically) not biologically relevant
...
+ **Size** is of biological interest. 
  + We standardize for size to obtain shape variables, but record it for subsequent analyses $^{1}$ .left[.footnote[1: One can perform GM analyses without recording digitizing 'scale', but in this case one has no inherent size estimate, and no notion of its effect on shape]
]]
...
<video src="LectureData/03.superimposition/Trans.mp4" width="640" height="480" controls></video>
---
...
<video src="LectureData/03.superimposition/Rot.mp4" width="640" height="480" controls></video>
---
...
<video src="LectureData/03.superimposition/procrustes-superimposition-v2.mp4" width="480" height="360" controls></video>
...
<video src="LectureData/03.superimposition/larvalMorph-head-gpagen-PrinAxes-FALSE.mp4" width="640" height="480" controls></video>
---
...
+ 2D data initially contain $2p$ variables: $p$ landmarks in $k=2$ dimensions
...
+ For 3D data, dimensionality is $3p-7$ (general equation: $pk – k – k(k – 1)/2 – 1$

+ These dimensions are redundant, so standard parametric statistics will not work (singular covariance matrix... means divide by zero)
