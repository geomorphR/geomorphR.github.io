...
Generalized Procrustes Analysis (GPA) removes non-shape information from landmark coordinates
  ...
  + Scalesall specimens to unit centroid size
  + Rotates ll specimens to minimize differences
...
+ $\mathbf{H}$ is found from a regression (i.e., fit) of $\mathbf{Y}_2$ on $\mathbf{Y}_1$
  + Perform `SVD` of: $\mathbf{Y}_1^T\mathbf{Y}_2=\mathbf{U\Sigma{V}}^T$
  + Then $\mathbf{H}=\mathbf{VSU}^T$

+ Here $\mathbf{S}$ has diagonals of $1$ and of the same sign
