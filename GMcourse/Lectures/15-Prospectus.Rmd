...
subtitle: "Prospectus for the Future"
...
- Our goals for the week were:
    ...
    - 3: Statisticaly evaluate hypotheses of shape variation and covariation using robust statistical methods
...
- This, then, **must** be coupled with **RRPP and effect size (Z-score) evaluation** for shape analysis:  $z =\small\frac{
\log\left( F\right) - \mu_{\log\left(F\right)}
} {
 \sigma_{\log\left(F\right)}
}$ 

# Recall the RRPP Procedure

- For virtually all shape-based hypotheses, we used permutation procedures (RRPP). This is embodied as:

1. Obtain pseudo values from predicted values and permuted residuals from reduced model as: $\small\mathbf{\mathcal{Y}} = \mathbf{\hat{Y}}^* + \mathbf{E}_{\left[s,\right]}^*$: repeat *many* times
2. Calculate summary statistic (e.g., $\small{F}$-value) in every random permutation to generate an  *empirical sampling null distribution* 
3. For $\small{N}$ permutations, $\small{P} = \small\frac{N(F_{random} \geq F_{obs})}{N}$
4. Calculate *effect size* as a standard deviate of the observed value in a normalized distribution of random values for comparing effects within and between models:
$z =\small\frac{
\log\left( F\right) - \mu_{\log\left(F\right)}
} {
 \sigma_{\log\left(F\right)}
...
Important! RRPP is *not* constrained by *n:p* ratios and in fact is quite useful when $\small{n}\ll{p}$ (displays high power in such cases)

###### Collyer et al. *Heredity.* (2015); Adams & Collyer. *Evolution.* (2016); Adams & Collyer. *Evolution.* (2018)

# Procrustes Paradigm + RRPP: Present Applications

What we learned this week (Procrustes + RRPP and $\small{Z}$ scores) provides the tools to evaluate an inordinately large breadth of biological hypotheses related to shape variation.  A partial list includes:

- 1: General linear model (GLM) questions: 
    - Do groups differ in shape (anova)?
...
# Procrustes Paradigm + RRPP: Present Applications (Cont.)
...
>- This implies that the values for each landmark of each specimen are derived from a circular normal distribution, which is indpendent landmark by landmark
...
>- Also, consider the possibility that some landmarks are correlated with others (spatially, developmentally, functionally), implying that changes in the positions of one landmark may be predicted by those of another (i.e., landmark changes are correlated)

>- Neither of these conditions is considered by GPA, and while GPA exhibits a fine 'null' model of shape variation, we may strive to develop a more realistic null model for shape variation, where correlated data may be accomodated
...
>- This assumes that all landmarks are 'equal' in the sense of their variation and covariation (and thus, all coordinates are equal or unweighted)
...
include_graphics("LectureData/15.prospectus/WGPATheobald.png")  
```

- Here (with simplifying assumptions), one can align protein structures allowing for different variation among 'landmarks' (but with equal variation within)
...
>- But what is the null hypothesis against which these patterns are compared?

# 2: Integration and Modularity: A Spatial Perspective

- Recent work of Bookstein (2015) highlighted the need to compare 'global' patterns of integration relative to a 'null' expectation

- This begs the question: shoudn't other patterns of integration and modularity (say, among sub-units) be evaluated relative to some 'null' expectation beyond that of the empirical sampling distribution derived from permuting landmarks into modules?
...
```{r, echo = FALSE, out.width="35%"}
include_graphics("LectureData/15.prospectus/SpatialCov.png")  
```
...
```{r, echo = FALSE, out.width="35%"}
include_graphics(c("LectureData/15.prospectus/SpatialCov.png","LectureData/15.prospectus/Mouse.png"))  
```
...
- Procedure
    ...
    - 3: Estimate pairwise $\small{z}$-tests across models following Adams and Collyer (2016). *Evol.*
    - 4: Assemble $\small{z}$-score matrix from 2 and 3     
    - 4: Use PCoA to visualize model space 
