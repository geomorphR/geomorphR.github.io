...
## And some propeties that make them special
...
|**Diagonal matrix:** A symmetric matrix that has non-0 elements only along the diagonal; all others are 0.| $\small{\mathbf{D} = \begin{bmatrix} 2 & 0 & 0 \\ 0 & 7 & 0\\ 0 & 0 & 0 \\ \end{bmatrix}}$ | Note that the diagonal elements can still be 0.  If all diagonal elements are equal, we can say $\mathbf{D} = a\mathbf{I}$, for a scalar, $a$, and an identity matrix, $\mathbf{I}$.|
|**Identity matrix:** A diagonal matrix that has diagonal elements equal to 1.| $\small{\mathbf{I} = \begin{bmatrix} 1 & 0 & 0 \\ 0 & 1 & 0\\ 0 & 0 & 1 \\ \end{bmatrix}}$ | This is the matrix equivalent to a value of 1, such that $\mathbf{AI} = \mathbf{A}$
---
...
| Matrix and description | Example | Comments |
| ------ | ---- | ------ |
|**Orthogonal matrix:** A symmetric matrix that has the property that $\mathbf{AA}^T = \mathbf{A}^T\mathbf{A} = \mathbf{I}$.| $\small{\mathbf{A} = \begin{bmatrix} 0.7071 & 0.7071 \\ 0.7071 & -0.7071 \end{bmatrix}}$ | $\small{\mathbf{AA}^T} = \begin{bmatrix} 0.7071 & 0.7071 \\ 0.7071 & -0.7071 \end{bmatrix} \begin{bmatrix} 0.7071 & 0.7071 \\ 0.7071 & -0.7071 \end{bmatrix} = \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}$ |
|**Orthonormal matrix:** An orthogonal matrix with unit vectors (length of 1) $\mathbf{AA}^T = \mathbf{A}^T\mathbf{A} = \mathbf{I}$.| $\small{\mathbf{A} = \begin{bmatrix} 0.7071 & 0.7071 \\ 0.7071 & -0.7071 \end{bmatrix}}$ | $\small{\lvert\lvert\mathbf{a}_1 \rvert\rvert  =(\mathbf{a}_1^T\mathbf{a}_1)^{1/2} = (0.7071^2 +0.7071^2)^{1/2} =1}$ ;  $\small{\lvert\lvert\mathbf{a}_2\rvert\rvert=(\mathbf{a}_2^T\mathbf{a}_2)^{1/2} = (0.7071^2 + (-0.7071)^2)^{1/2} =1}$|
|**Idempotent matrix:** A symmetric matrix with the property $\mathbf{HH} = \mathbf{H}$ | $\small{\mathbf{H} = \begin{bmatrix} 0.5 & 0.5 \\ 0.5 & 0.5 \end{bmatrix}}$ | $\begin{align*} \small{\mathbf{HH}} = \begin{bmatrix} 0.5 & 0.5 \\ 0.5 & 0.5 \end{bmatrix} \begin{bmatrix} 0.5 & 0.5 \\ 0.5 & 0.5 \end{bmatrix} \\ = \small{\begin{bmatrix} 0.5^2 + 0.5^2 & 0.5^2 + 0.5^2 \\ 0.5^2 + 0.5^2 & 0.5^2 + 0.5^2 \end{bmatrix} = \begin{bmatrix} 0.5 & 0.5 \\ 0.5 & 0.5 \end{bmatrix}} \end{align*}$  
...
$$\mathbf{H}_{orthogonal} = \begin{bmatrix} \cos\theta & -\sin\theta\\ \sin\theta & \cos\theta \end{bmatrix}$$
...
$$MS= trace (\Delta\mathbf{H}\mathbf{Y}) / (k_{alternative} - k_{null})$$
...
$$\mathbf{A}^{-1} = \left| \mathbf{A} \right|^{-1} \begin{bmatrix}
d & -b\\ -c & a \end{bmatrix} = 0.5 \begin{bmatrix} 6 & -4\\ -4 & 3
\end{bmatrix} = \begin{bmatrix} 3 & -2\\ -2 & 1.5 \end{bmatrix}$$
---
...
$$\mathbf{Z}=\mathbf{X}\mathbf{\beta } $$
$$\mathbf{X}^T\mathbf{Z} = \mathbf{X}^T\mathbf{X}\mathbf{\beta } $$

$$\left( \mathbf{X}^T\mathbf{X} \right)^{-1} \mathbf{X}^T\mathbf{Z} = 
\left( \mathbf{X}^T\mathbf{X} \right)^{-1} \mathbf{X}^T\mathbf{X}\mathbf{\beta } $$
$$\left( \mathbf{X}^T\mathbf{X} \right)^{-1} \mathbf{X}^T\mathbf{Z} = 
\mathbf{I}\mathbf{\beta } $$

$$\hat{\mathbf{\beta }}=\left ( \mathbf{X}^{T} \mathbf{X}\right )^{-1}\left ( \mathbf{X}^{T} \mathbf{Z}\right )$$
...
$\mathbf{U}$ (left singular values) and $\mathbf{V}$ (right singular values) are solved via numerical approximation algorithms.  The number of singular values is $p'= \min(n,p)$. If $\mathbf{Y}$ is not full-rank, some of the singular values in $\mathbf{D}$ will be 0.  The singular values decrease from largest to smallest.
...
$$\mathbf{S}_{p \times p} =\mathbf{U}_{p \times p}  \mathbf{U}_{p \times p}^T$$

where $\mathbf{U}$ is a triangular matrix.
