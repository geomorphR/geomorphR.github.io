...
### Linear Algebra: Single Goal (maybe not realized in this lecture, but in this week)

Understand why these equations are foundational, and used in geometric morphometrics (GM) and multivariate statistics:
...
### Mike's simple definition of a matrix

### *A matrix is a template for algorithms used on vectors.*
...
For the product to work, both matrices must have the same numer of rows, $n$.
---
...
- A matrix **.blue[cross-product]** of the form, $\mathbf{X}^T\mathbf{X}$ is a generalized method of squaring.  This matrix has summed squared values of column vectors of $\mathbf{X}$ on a diagonal vector and summed cross-products between vectors in the off-diagonal elements.
- A matrix **.blue[outer-product]** of the form, $\mathbf{X}\mathbf{X}^T$ can also be considered a generalized method of squaring.  This matrix has summed squared values of row vectors of $\mathbf{X}$ on a diagonal vector and summed cross-products between vectors in the off-diagonal elements.
- The **.blue[trace]** of a symmetric matrix is the sum of diagonal elements.  One convenience is $trace(\mathbf{X}^T\mathbf{X}) = trace(\mathbf{X}\mathbf{X}^T)$.
---

### Important Matrix and Vector Multiplication Properties (cont.)

- A matrix is **.blue[square]** when it has equal rows and columns, e.g., $n \times n$.
...
---
Let's revisit this, focusing on the left sides:
...
Let the matrix, $\mathbf{A}$, suggest ***association*** or ***alignmnet***.

Let the matrix, $\mathbf{Z}$, be a manipulated form of the data matrix, $\mathbf{Y}$.  (It might be unmanipulated)

** $\mathbf{AZ}$ or $\mathbf{ZA}$ could represent projection.** So, let $\mathbf{A} = \mathbf{H}$, a **projection matrix**.  (A projection matrix is often called a "Hat" matrix.)
---
...
.footnote[
$^*$ LAPACK has roots in `Fortran`, which is used in `R`.  `R` also relies on `C` and `C++`, which also use LAPACK plus other linear algebra packages.
]
---
...
Note, the second one *.blue[can be written]*
...
#### An Example (recalling an earlier example) using R scripts
If we now substitute $\mathbf{X}$ for $\mathbf{X}_0$, this is the result ($\mathbf{H}_A\mathbf{Y}$)
.pull-left[
.med[
```{r, include = TRUE, echo = TRUE}
...
### Call back to Matrix Projections!
...
$$\mathbf{H}_{A}\mathbf{Y} - \mathbf{H}_{0}\mathbf{Y} = \Delta\mathbf{H}\mathbf{Y}$$
...
Let $\mathbf{Z}$ be a transformation of the data matrix, $\mathbf{Y}$.  (Could be mean-centering of the data, scaling of the data, or normalizing the data.)

Let $\mathbf{H} = \mathbf{X} \left( \mathbf{X}^T\mathbf{X} \right)^{-1}\mathbf{X}^T$ be a **projection** matrix for a linear model design, $\mathbf{X}$.  This matrix projects (predicts) what values of $\mathbf{Z}$ should be, based on $\mathbf{X}$.

Then, $\mathbf{HZ}$ are the estimates a linear model makes!
...
$$\mathbf{Z}_{n \times p} =\mathbf{U}_{n \times p'} \mathbf{D}_{p' \times p'} \mathbf{V}_{p' \times p'}^T$$

$\mathbf{U}$ (left singular vectors) and $\mathbf{V}$ (right singular vectors) are solved via numerical approximation algorithms.  The number of singular values is $p'= \min(n - 1,p)$. If $\mathbf{Y}$ is not full-rank, some of the singular values in $\mathbf{D}$ will be 0.  The singular values decrease from largest to smallest.
...
---
Let's revisit this, focusing on the right sides:
...
Let the matrix, $\mathbf{U}$, be a set of orthogonal vectors, related to $\mathbf{A}$, found via SVD.

Let the matrix, $\mathbf{V}$, be a set of orthogonal vectors, related to $\mathbf{Z}$, found via SVD.

Let the matrix, $\mathbf{D}$, be a diagonal matrix with scalars describing the strength of association between $\mathbf{U}$ and $\mathbf{V}$ (and thus, $\mathbf{A}$ and $\mathbf{Z}$), found via SVD.

**So, for example, the left-hand side might be estimated values from a liner model and the right hand side might find the principal components of the estimated values.**
...
**So, for example, the left-hand side might be alignment of two separate data matrices and the right hand side might describe the strength of association (via the singular values, $\mathbf{D}$).**
...
The left describes either multivariate data (like GM data) or an alignment of the multivariate data, say to a linear model or other set of data.  The right describes a process for finding major vectors in a multivariate space, plus interpreting how important the vectors are.
