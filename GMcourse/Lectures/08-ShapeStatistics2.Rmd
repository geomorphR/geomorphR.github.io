...
One might realize that there are a finite set of random possibilities, at least based on a sample of data, but enumerating all possibilities is perhaps not possible, or at least excrutiating.
...
|Statistic|Calculation|Type|OLS OK|GLS OK| Comment|
|---|-----|---|--|--|-------|
| Vector MD | $\small MD = \lvert d_{{12}_A} - d_{{12}_B}\rvert$ | RRPP second order| Yes | Yes | For two groups, $A$ and $B$, contrast the difference between consistent estimates for states 1 and 2.  **This can only be done with RRPP.**|
| Vector Correlation or Angle | $\small VC = \frac{\mathbf{v}_A^T\mathbf{v}_B}{\lvert\lvert \mathbf{v}_A\rvert\rvert \lvert\lvert \mathbf{v}_B\rvert\rvert}$ $\small \theta = \cos^{-1}VC$ | RRPP second order| Yes | Yes | For two groups, $A$ and $B$, find the vector correlation or angular difference between vectors that consistently estimate changes from state 1 to state 2, or slopes associated with a covariate.  **This can only be done with RRPP.**  Note that $\mathbf{v}$ means $(\mathbf{x}_1^T\hat{\boldsymbol{\beta}} - \mathbf{x}_2^T\hat{\boldsymbol{\beta}})$ and $\lvert\lvert \mathbf{v}\rvert\rvert$ is the length of $\mathbf{v}$.|
|Trajectory MD| $\small MD = \lvert \sum d_A - \sum d_B \rvert$ | RRPP second order| Yes | Yes | For two groups, $A$ and $B$, find the difference in *path distances* connecting estimates from state 1 to state 2 to state 3 to ... **This can only be done with RRPP.**  This is a component of **trajectory analysis.**
...
|Statistic|Calculation|Type|OLS OK|GLS OK| Comment|
|---|-----|---|--|--|-------|
| Trajectory Correlation or Angle | $\small VC = \frac{\mathbf{v}_A^T\mathbf{v}_B}{\lvert\lvert \mathbf{v}_A\rvert\rvert \lvert\lvert \mathbf{v}_B\rvert\rvert}$ $\small \theta = \cos^{-1}VC$ | RRPP second order| Yes | Yes | For two groups, $A$ and $B$, find the vector correlation or angular difference between first PC vectors found separately for each group.  **This can only be done with RRPP.**  
| Trajectory Shape (Procrustes distance)| $PD = \left[(\mathbf{z}_A-\mathbf{z}_B)^T(\mathbf{z}_A-\mathbf{z}_B)\right]^{1/2}$ |RRPP second order| Yes | Yes | For two groups, $A$ and $B$, find the Procrustes distance between two vectors of estimates, $\mathbf{z}_A$ and $\mathbf{z}_B$, which were obtained from GPA on trajectory points.  **This can only be done with RRPP.**  
...
.remark-code[.red[Look at the second row of the cofficients above.  The sign of the values does not matter.  It's the square of the values and whether they are quite different from 0 that confirms a significant amount of shape change associated with size change.]]
---
...
What this example shows is the GLS estimation gets us to the same place, without assuming observations are independent.  The statistics changed a little (but allometry was not significant).  There might be cases where a significant effect **BECAUSE** of phylogenetic relatedness is redder non-significant after accounting for the non-independence of observations.
...
Biological hypotheses would be boring if they were always $\mathbf{H}_{mean}$ versus $\mathbf{H}_{somthing}$.
...
+ Let $\mathbf{H}_{full}$ be any model that is comapred to a reduced model, having some parameters that the reduced model lacks.
...
.footnote[Mixed effects in a linear model are beyond the scope of this workshop, but we could probably help somone with such needs in lab time.]
---
...
Be weary of AIC scores or the likelihoods that generate them when calculated at the limit of possible data dimenisons, because $p > n$.
...
First we have to fit a model that allows estimating of unique allomteric trends.
...
### RRPP extensions: comparing morphological disparity among groups
Test the null hypotheses: $\sigma^2_{1} = \sigma^2_{2}=...$, where $\sigma^2$ is variance of points scattered aroun a group mean.  This is a measure of how variable in shape each group is.
...
**RRPP allows calculation of second-order statistics like these in every random permutation.**
