...
+ Fitted values (transformed, if GLS), based on coefficients, $\hat{\mathbf{Z}} = \tilde{\mathbf{X}}\hat{\boldsymbol{\beta}} = \tilde{\mathbf{H}}\tilde{\mathbf{Z}}$.  
+ Residuals (transformed) can also be found as $\tilde{\mathbf{E}} = \tilde{\mathbf{Z}} - \hat{\mathbf{Z}} = \left(\tilde{\mathbf{I}} - \tilde{\mathbf{H}} \right) \tilde{\mathbf{Z}}$.)
...
### Let's simplify things before going forward!

+ $\mathbf{Z}$ represents the data ($\mathbf{Y}$ or $f(\mathbf{Y})$)
+ $\mathbf{X}$ represents a linear model
+ $\mathbf{H}$ also represents a linear model, but specifically for estimating values from the model.  ($\boldsymbol{\hat{\beta}}$ is embedded within $\mathbf{H}$.)
+ $\tilde{}$ is used to remind us if GLS estimation was used (can be ignored if OLS estimation).
+ The subscripts $_{alt}$ or $_{full}$ represent models with more parameters then $_{null}$ or $_{reduced}$.  The parameters of the latter are also found within the former.
+ A **term** represents one or more parameters added to $\mathbf{X}_{null}$ to achieve $\mathbf{X}_{alt}$.  For example, adding a $Group$ as a term might require adding many parameters to define different group levels.

+ **Hypothesis tests compare estimates made by $\mathbf{H}_{alt}$ and $\mathbf{H}_{null}$.  If estimates are similar, the models are similar.  If estimates are different, the additional parameters in $\mathbf{H}_{alt}$ are important (significant).**
...
Yup, that is 10,000 RRPP permutations in a few seconds.  We will understand what happened soon.
---
...
We would allow with 95% confidence that a vector length of 0.047 could be obtained by chance.  Beyond this, we reject the null hypothesis that there is no shape change associated with size change.  The observed vector length of 0.11 is much larger than 0.047, such that the observed value is 4.14 $SD$ greater than expect by chance.
...
3. Coefficients, via MANOVA statistics
...
|In words | No group means are different | At least one group mean differs from one other group mean.|
...
|Model|Terms|Purpose|
|---|------|------------|
|`fit6`| $\log CS$ + Sex + $\log CS$:Sex| Sexual dimorphism, unique allometry.|
|`fit7`| $\log CS$ + Pop + $\log CS$:Pop| Ecological divergence, unique allometry.|
...
fit6, fit7, fit8, fit9, fit10, type = "logLik", tol = 0.01, pc.no = 5)
```
]
...
2. Shape analyses involve aligning data to some kind of matrix or vector, $\mathbf{A}\mathbf{Z}$.
...
+ **In all cases, $\mathbf{A}\mathbf{Z}$ is the starting point.  $\mathbf{Z}$ is the matrix of shape data.  Understanding what $\mathbf{A}$ is helps inform what the analysis intends to do.**
...
3. If you can appreciate that every statistical analysis is a permutation of $\mathbf{A}\mathbf{Z}$ and every statistical test is an application of RRPP, using an appropriate null model, you are on your way to taking ownership of your data!
