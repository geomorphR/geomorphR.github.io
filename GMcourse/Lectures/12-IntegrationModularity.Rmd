...
+ Some considerations when embarking on a trait covariation study are:
  ...
  + Does $H_0$ differ when testing overall integration versus integration among subsets? 

##### As we will see, the empirical answer to the last few questions is found via RRPP
---
...
VE | $\frac{{\sum}{\left(\lambda_i-\overline{\lambda}\right)^2}}{p}$ | Wagner, 1984
$V_{rel}$ | $\frac{{\sum}{\left(\lambda_i-\overline{\lambda}\right)^2}}{p(p-1)\overline{\lambda}{^2}}$ | Pavlicev et al. 2009
...
### 2B: Comparing Integration Across Datasets (Cont.)
...
### 3: Quantifying Modularity: $CR$ Coefficient
...
### 3B: Comparing Modularity Across Datasets
