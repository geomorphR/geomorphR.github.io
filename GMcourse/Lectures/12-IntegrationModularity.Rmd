...
### Quantifying Integration and Modularity: Starting Points $^1$

+ Integration and modularity concern trait covariation

+ Mathematically, this requires estimating the trait covariance matrix:

$$\hat{\mathbf\Sigma} = \mathbf{Y_c^T}\mathbf{Y_c}/ (n-1)$$

+ The $n:p$ ratio is *critical* here: one must have sufficient observations relative to the number of variables for a reliable estimate of $\hat{\mathbf\Sigma}$

+ When this is not the case (and for GM data generally), $\hat{\mathbf\Sigma}$ is singular, causing downstream mathematical challenges for some methods

.footnote[1: Must-read paper relating random matrix theory to covariance estimation: Bookstein (2017). *Evol. Biol.*]
...
+ All approaches decompose information in $\hat{\mathbf{\Sigma}}$
  + Estimating $\hat{\mathbf{\Sigma}}$ requires sufficient $n$ vs $p$

+ Summary measures must be robust to $n$ and $p$ (see RV vs. CR)
